...
contract Condominium is ICondominium {
    ...
    mapping(uint16 => bool) residences; //units => true
    ...
    mapping(uint16 => uint) private _nextPayment; //unit/apart => next payment (timestamp in seconds)

    Lib.Topic[] public topics;
    mapping(bytes32 => uint) public _topicIndex; //topic hash => array index
    ...
    function populateResidences(
        ...
        for (uint16 i = 1; i <= totalBlocks; i++) {
            for (uint16 j = 1; j <= totalFloors; j++) {
                for (uint16 k = 1; k <= totalUnits; k++) {
                    uint16 id = uint16(i * 1000 + j * 100 + k);
                    residences[id] = true;
                }
            }
        }
    ...
    modifier onlyResident() {
        if (tx.origin != manager) {
            ...
            require(
                block.timestamp <= resident.nextPayment, //+ (30 * 24 * 60 * 60),
    ...
    function removeResident(address resident) external onlyManager {
        ...
        if(index ==0){
            require(residents[index].wallet == resident, "The resident does not exist");
        }
    ...
    function getTopics(
        ...
        return Lib.TopicPage({topics: result, total: residents.length});
    }
    ...
    function closeVoting(
        ...
        require(
            numbersOfVotes(title) >= minimumVotes,
    ...
    function payQuota(uint16 residenseId) external payable {
        ...
        if (_nextPayment[residenseId] == 0) {
            _nextPayment[residenseId] = block.timestamp + _thirtyDays;
        } else {
            _nextPayment[residenseId] += _thirtyDays;
        }
    }
